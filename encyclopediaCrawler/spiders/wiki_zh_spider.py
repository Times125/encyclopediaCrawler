...
class WikiZHSpider(RedisCrawlSpider):
    ...
    task_queue = spider_args['wiki_zh_task_queue']
    base_url = "https://zh.wikipedia.org"
    name = spider_args['wiki_zh_spider_name']
    ...
    def parse(self, response):
        ...
        ftb = lambda x: '-'.join(x[:3]) + ' ' + ':'.join(x[3:])
        items['update_time'] = ftb(re.compile(r'([\d]+)').findall(latest.get_text())) if latest else None
        # 词条抓取时间
